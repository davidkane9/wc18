...
This measure captures the fact that teams like England, France, Spain and Germany are likely to do well while teams like Iran, Panama and Peru are not. Russia, as the host country, is a more difficult case. 

There are several problems with this analysis:

1. The league variable is at the country level and does not distinguish among different leagues within a given country. For example, in ENG, the best league (Premier League) is much better than the second tier (Championship League), but we don't know which applies for a given player.
2. The measure is biased in favor of countries with elite leagues since they will find it easy to source most/all of their players from their "home" leagues. It is also biased toward teams from "nearby" countries.
3. The measure is biased against countries with good (but note elite) leagues, which often have good players/teams. For example, one reason that Brazil is not ranked near the top is that it has two players who star for each of FC Shakhtar Donetsk and SC Corinthians, two good teams which play in a mediocre leagues (in Ukraine and Brazil).

Better approach


David Kane teaches at [Harvard University[(www.harvard.edu)] and co-organizes the [Boston R User Group](https://www.meetup.com/Boston-useR/).
